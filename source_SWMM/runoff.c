...
static void   runoff_addLeak(int subcatch, double tStep);
...
void runoff_addLeak(int subcatch, double tStep)
//  Input:   j = subcatchment index
//           tStep = time step (sec)
//  Output:  None
//  Purpose: Add groudwater leakage to pervious subarea depth
{
 TSubarea     *b_subarea;
 TGroundwater *b_gw;
 double depth_leak;
 b_subarea         = &Subcatch[subcatch].subArea[1];
 b_gw              = Subcatch[subcatch].groundwater;
 depth_leak        = b_gw->gwLeak / (24.0 / (tStep / 60.0 / 60.0));
 b_subarea->depth += depth_leak;
}
