...
//BB
double     Vall_infil;    // non-LID infiltration unlimited by GW
...
// BB
static void   updatePondedDepth_BB(TSubarea* subarea, double* tx);
static double findSubareaRunoff_BB(TSubarea* subarea, double tRunoff);
...
//
{
    ...
    for (i = IMPERV0; i <= PERV; i++)
    {
        ...
        // BB
        Subcatch[j].subArea[i].BB_depth = 0.0;
        Subcatch[j].subArea[i].BB_inflow = 0.0;
        Subcatch[j].subArea[i].BB_runoff = 0.0;
    }
...
//
{
    ...
    double gwleak = Subcatch[j].oldRunoff * 900;
    ...
    VlidDrain = 0.0;
    VlidReturn = 0.0;
    // maintain mass balance; though last time step DOES NOT WORK
...
////
    ...
    //BB - in case using my BB update runoff:
    //massbal_updateRunoffTotals(RUNOFF_RUNOFF, vOutflow + gwleak);
...
//
{
    ...
    TGroundwater *gw_struct;           // pointer to groundwater structure

    //BB
    double    all_inf = 0.0;           // infil rate (ft/sec) unlimited by GW
    double    ind_runoff = 0.0;        // runoff rate (ft/sec) independent of GW head
...
// --- compute infiltration loss rate
    if (i == PERV) {
     ...
     all_inf = Subcatch[j].infilAvail;
     }

    // --- add precip to other subarea inflows ------------- NEEED TO GET A COPY OF STRUCTURE
    subarea->inflow += precip;

    /* BB -- add any groundwater leakage (positive head>land) to subarea inflows
    gw_struct = Subcatch[j].groundwater;

    gw_struct ->gw_leak = (gw_struct->waterTableElev - gw_struct->surfElev);
    if (gw_struct->gw_leak > 0) {subarea->inflow += (gw_struct->gw_leak/area/(24*3600));}
    //*/
    //subarea->inflow +=
    ...
    {
        // new subarea variables (in same structure)
        // might not need to copy but need to initialize elsewhere
        // regular calculations for ET ...
        subarea->BB_inflow = subarea->inflow;
        subarea->BB_depth  = subarea->depth;

        subarea->inflow -= surfEvap + infil;
        updatePondedDepth(subarea, &tRunoff);

        //BB - gw not limiting conditions for RUNOFF; use with find subareaRunoff

        subarea->inflow_allinf -= surfEvap + all_inf;
        updatePondedDepth_BB(subarea, &tRunoff);
    }

    // --- compute runoff based on updated ponded depth for NO gw limitation
    //runoff = findSubareaRunoff_BB(subarea, tRunoff);
    // BB - ORIG:
...
//=============================================================================
void updatePondedDepth_BB(TSubarea *subarea, double* dt)
//
//  Input:   subarea = ptr. to a subarea,
//           dt = time step (sec)
//  Output:  dt = time ponded depth is above depression storage (sec)
...
//
 {
 double ix = (subarea->inflow_allinf);       // excess inflow to subarea (ft/sec)    //(5.1.008)
 double dx;                         // depth above depression storage (ft)
 double tx = *dt;                   // time over which dx > 0 (sec)

                                    // --- see if not enough inflow to fill depression storage (dStore)
 if (subarea->BB_depth + ix*tx <= subarea->dStore) {
  subarea->BB_depth += ix * tx;
  }

 // --- otherwise use the ODE solver to integrate flow depth
 else {
  // --- if depth < dStore then fill up dStore & reduce time step
  dx = subarea->dStore - subarea->BB_depth;
  if (dx > 0.0 && ix > 0.0) {
   tx -= dx / ix;
   subarea->BB_depth = subarea->dStore;
   }

  // --- now integrate depth over remaining time step tx
  if (subarea->alpha > 0.0 && tx > 0.0) {
   //theSubarea = subarea;
   odesolve_integrate(&(subarea->BB_depth), 1, 0, tx, ODETOL, tx,
                      getDdDt);
   }
  else {
   if (tx < 0.0) tx = 0.0;
 ...
 // --- do not allow ponded depth to go negative
 if (subarea->BB_depth < 0.0) subarea->BB_depth = 0.0;
 ...
 *dt = tx;
 ...
 double findSubareaRunoff_BB(TSubarea* subarea, double tRunoff)
 //
 //  Purpose: computes runoff (ft/s) from subarea after current time step.
 //  Input:   subarea = ptr. to a subarea
 //           tRunoff = time step over which runoff occurs (sec)
 ...
 //
 {
     double xDepth = subarea->BB_depth - subarea->dStore;
     double runoff = 0.0;
