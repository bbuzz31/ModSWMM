...
def extract(filename, *labels):
    ''' Get the time series data as a particular object and variable
...
#@mando.command
def extract_BB(filename, *labels):
    ...
    '''
    obj     = SwmmExtract(filename)
    cell_ts = np.zeros((obj.nperiods, len(labels)))
    names   = []
    for i, label in enumerate(labels):
        ...
        for j, time in enumerate(range(obj.nperiods)):
            date, value = obj.GetSwmmResults(
                typenumber, name, int(variableindex), time)
            cell_ts[j][i] = value


    begindate = datetime.datetime(2012, 06, 30, 00)

    dates     = pd.date_range(begindate, periods=len(cell_ts), freq='H')
    df_all    = pd.DataFrame(cell_ts,  columns=names, index=dates)

    return df_all

def getdates(filename):
    SwmmExtract(filename).GetDates()

# BB
def extract_1d(filename, *labels):
    """
    Get the time series data for a particular object and variable
