...
import os
import os.path as op  # convenience
from os import remove # Required to clear info file.

import math, re # Used to create .rpt and .out paths
# ------------------------ CONSTANTS ---------------------------
# path to SO
SO_loc  = os.path.join(op.expanduser('~'), 'Software_Thesis', 'SWMM', 'swmm_so',
                                             'source5_1_011', 'swmm5')
### identical to original engine
#SO_loc  = os.path.join(op.expanduser('~')'Software_Thesis', 'SWMM', 'swmm_so',
                                                    #   'source_orig', 'swmm5')
...
_attribute_constants = (DEPTH, VOLUME, FLOW, SETTING, FROUDE, INFLOW, FLOODING,
                        ...
                        POND, AVAIL_INFIL, LINK_AREA, LEAK, SUB_AREA, GET_LEAK)
