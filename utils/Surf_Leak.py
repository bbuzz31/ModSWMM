...
class surf_leak(ss_base):
    ...
    def plot_ts_leak_1(self, row, col, slr=2.0):
        """ Plot ts of leakage at one location, 1 scenario """
        arr_leak = self._load_uzf('surf_leak')[slr][:, row, col]
        ser_leak = pd.Series(arr_leak, index=self.ts_day, name='Leakage')
        fig, axes = plt.subplots()
        title     = fig.suptitle('Surface Leakage, Row: {}, Col: {}'.format(row+1, col+1))

        # BB.print_all(ser_leak)
        ser_leak.plot(ax=axes)
