...
C
      ...
      ! --BB--
      LOGICAL modflow_is_done, step_done, step_fail
C
...
C          WRITE(*,25)KPER,KSTP
C   25     FORMAT(' Solving:  Stress period: ',i5,4x,
C     &       'Time step: ',i5,4x,'Groundwater-Flow Eqn.')
...
C-----END OF TIME STEP (KSTP) AND STRESS PERIOD (KPER) LOOPS
   90   CONTINUE
        ! --BB--
        step_done = modflow_is_done ('mf_done', KKPER-1)
  100 CONTINUE
C
C
...
C10-----END OF PROGRAM.
     ...
     1          NCVGERR,' TIME(S)'
        ! --BB--
        step_fail = modflow_is_done('mf_fail', KPER-1)
      ELSE
