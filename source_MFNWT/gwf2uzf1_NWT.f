...
C     ARGUMENTS
C    ------------------------------------------------------------------
      INTEGER Iunithuf, Iunitlpf, Iunitupw
C    ------------------------------------------------------------------
C    LOCAL VARIABLES
C    ------------------------------------------------------------------
...
C     BB VARIABLES
      INTEGER(4) delay
      LOGICAL swmm_is_done, checkswmm
...
C       STRESS PERIOD.
      ...
      IF ( nuzf.LT.0 ) THEN
        ...
        WRITE(*,9015)Kkper
 9015   FORMAT(/,' BB-WARNING: NO FINF FILE FOR STRESS: ',i5,6x,/)
      ELSE
C       BB Look for file indicating that SWMM is done (FINF AND PET overwritten)
        IF (Kkper.GT.1) Then
         checkswmm = swmm_is_done(Kkper, delay)
        END IF
C
...
C        IRUNFLG IS NOT EQUAL TO ZERO.
 ...
 9004     FORMAT (1X/, 'USING ET RATE FROM PREVIOUS STRESS ',
     +            'PERIOD. CURRENT PERIOD IS: ', I7)
          WRITE(*,9016)Kkper
 9016     FORMAT(/,' BB-WARNING: NO PET FILE FOR STRESS: ',i5,6x,/)
        ELSE
C
...
! Suppress seepout beneath a lake
     ...
     +               csepmx-csep.GT.CLOSEZERO ) THEN
                      ij = Icell(ic, ir, il)
! Derivative for RHS
                      A(IA(ij)) = A(IA(ij)) + (csepmx/SURFDEP)*celtop
! Derivative for HCOF
                      A(IA(ij)) = A(IA(ij)) - (h*csepmx/SURFDEP)
                    END IF
                  END IF
                END IF
              END IF
...
C--------SUBROUTINE SGWF2UZF1OLF
      SUBROUTINE SGWF2UZF1OLF(Iunitsfr, Iunitlak, Iunitswr, Igrid)
C     ******************************************************************
...
C        VALUE IS SPECIFIED FOR A LAKE.
      ...
      END IF
C
C3A----IF PRMS ACTIVE THEN RETURN
...
C-------------LAK REACHES
      ...
      END DO
C
Ctemp------add runoff only to last reach in a segment for P.Juckem
      !IF ( Iunitsfr.GT.0 ) THEN
      !  irchcheck = 0
      !  nsegold = 1
      !  DO i = 1, NSTRM
      !    nseg = ISTRM(4, i)
      !    if ( nsegold.NE.nseg ) irchcheck = 1
      !    nsegold = nseg
      !    seglen = SEG(1, nseg)
      !    gwrunof = SEG(26, nseg)
      !    rchlen = STRM(1, i)
      !    IF ( ISEG(4, nseg).EQ.irchcheck ) STRM(24, i) = gwrunof
      !    irchcheck = irchcheck + 1
      !  END DO
      !END IF
C
C5------PROPORTION RUNOFF TO REACHES ON BASIS OF STREAM LENGTH.
      IF ( Iunitsfr.GT.0 ) THEN  !rgn original code commented out for Paul Juckem
        DO i = 1, NSTRM
          nseg = ISTRM(4, i)
          seglen = SEG(1, nseg)
          gwrunof = SEG(26, nseg)
...
C     BB Variables
      LOGICAL modflow_is_done, step_done
